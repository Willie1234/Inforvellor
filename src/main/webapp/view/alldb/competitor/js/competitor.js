...
function addCompetitor() {
	...
	updateJsp(flag);
	setHtmlParam(null);
	openDivArtDialog("添加竞争对手", 'showEcuadorCompetitorDlg',
...
function updateCompetitor() {
	...
	if (rows.length == 1) {
		setHtmlParam(rows);
		openDivArtDialog("市场分析", 'showEcuadorCompetitorDlg',
...
function updateJsp(flag){
	if (flag) {
		$("#titleSpan").html("修改我的竞争对手");
		$("#sureSpan").hide();
		$("#updateSpan").show();
	} else {
		$("#titleSpan").html("添加我的竞争对手");
		$("#sureSpan").show();
		$("#updateSpan").hide();
	}
}
...
function loadMethod(data) {
	...
	if (data.rows.length == 0) {
		$.messager.alert("提示", "未查询到竞争对手的数据!", 'info');
	} else {
		flagPage = data.flag;
...
function sfenxi(v) {
	...
	openDivArtDialog("市场分析", 'qkCompeterDiv',
...
function allCountryOptionShow() {
	...
	$("#sps").empty();
	$("#sps").append("<input type=\"checkbox\" onClick=\"selectCompAllChangeClick(this,'equirementSelects')\" name=\"all\"><span>选择全部</span><br/>")
