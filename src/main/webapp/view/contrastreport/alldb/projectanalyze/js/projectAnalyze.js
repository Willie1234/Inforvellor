...
function addProduct() {
	...
	flag = 1;
	checkInputByNum('');
	openDivArtDialog('添加产品', 'addProductTag', 'addProduct', 650, 400, true);
}
...
function updateproduct() {
	...
	if (rows.length == 1) {
		...
		$.messager.alert("提示", "请选择要编辑的产品,只能选择一个", 'info');
	}
}
...
function checkProjuctFromValue() {
	...
	if (verifycode.trim() == "") {
		$.messager.alert("提示", "标签名称不能为空", 'info');
		return false;
	}
	if (hscode2.trim() == "") {
		$.messager.alert("提示", "HS编码不能为空", 'info');
		return false;
	} else {
		var test = /^\d{6,8}$/;
		if (!test.test(hscode2)) {
			$.messager.alert("提示", "HS编码必须是6位或者8位的数字!", 'info');
			return false;
		}
	}
	if (productName.trim() == "") {
...
//			$.messager.alert("提示", "对不起，请输入英文，字符串开头不能有空格，可添加多个关键字，必须以单个空格分隔！",
//					'info');
//			return false;
//		}
//	}
...
function loadMethod(data) {
	if (data.rows.length == 0) {
		$.messager.alert("提示", "对不起!没有查询到符合您条件的记录数...", 'info');
	} else {
		flagPage = data.flag;
	}
}
...
function fenxi(hscode, goodsdescription) {
	...
	// 加载国家
	openDivArtDialog("市场分析", 'quankuDiv', 'quankuDivdsa', 950, 600, true);
}
...
//	emptyDateGrid($('#bquyerCompetitorsdg'));
	...
	openDivArtDialog("竞争对手", 'bqCompetitors', 'dbAllCompetitor', 1000,520,
			true);
}
...
function bqaddOpss(value, rowData, rowIndex) {
	...
	var jy = '<img src="'
			...
			+ rowData.exporter + "','" + rowData.country + '\')">查看采购商</a>';
	return fx + "&nbsp;&nbsp;&nbsp;&nbsp;" + jy;
}
...
function showCustomerNameForAllDB(importerName){
	if (importerName) {						// 进口商不为空
		...
		openDivArtDialog("添加客户信息", 'showCustomerDlg', 'showCustomerDlg', 800, 450,true);
	...
	if (id && country) {	//如果不为空
		$.ajax({
			...
			success : function(data){
			    ...
			    openDivArtDialog("查看详情", "detailmessageDIV", "1detailmessageDIV",900,600,true);
			}
		});
	}
}
...
function callCountryOption(){
	...
	$("#bqspanel").empty();
	$("#bqspanel").append("<input type=\"checkbox\" onClick=\"bqselAll(this)\" name=\"all\"><span>全选</span><br/>")
...
//		showAllDbCountrySel("dbAll_CompetitorCountry");
		openDivArtDialog("添加我的竞争对手", 'showEcuadorCompetitorDlg', 'closeCompetitorDialogId',800, 350,true);
	} else {
		$.messager.alert('提示','公司名称不能为空','info');
	}
}
...
function queryProcurement(companyName,country){
	if(!companyName){
 		$.messager.alert('提示','"对不起!公司名称为空无法查询对应的采购商！"','info');
 		return;
 	}
	// 查询采购商
    	...
    	pagination:true,
		    ...
		    onLoadSuccess: function(data){
		    	if(data.rows.length == 0){
		    		...
		    		$.messager.alert("提示","对不起!没有查询到对应的采购商...",'info');
			    } else {
...
//			CheckDataOver($('div[name="clientProgress"]'));
	...
	openDivArtDialog('买家资源库表', 'mybuyerDlg', 'mybuyer',850, 500, true);  
}
...
//			CheckDataOver($('div[name="clientProgress"]'));
			if(data.rows.length == 0){
				$("#buyerdg").hide();
				$.messager.alert('提示',"对不起，你输入的查询条件未检索到结果，请尝试更换条件值!",'info');
			}
		}
	});
}
