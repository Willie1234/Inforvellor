...
public class SelectFieldUtil
{
	...
	public static <T>String fmtHtml(String country,Map map,String selectFieldId,String selectFieldName) 
	{
		...
		if (map.containsKey(country+DataSearchConstantUtil.RESULTHIDEZHNAME)) {
			//查询结果隐藏字段中文名
			hideZhName = map.get(country+DataSearchConstantUtil.RESULTHIDEZHNAME).toString().split(",");
		}
	    ...
	    //该国家所有的列
		String[] allFieldName = (String[]) ArrayUtils.addAll(displayZhName, hideZhName);
		//html中的下标
		int index = 0;
		//参数列表
