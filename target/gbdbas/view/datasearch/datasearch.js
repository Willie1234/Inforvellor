...
function formatSearch(value,rowData,rowIndex)
{
	if(value != null && value.trim() !="NULL" && value.trim() !="null" && value.trim() !="" && value !="undefined")
	{
		...
		return "<a href=\"http://www.baidu.com/s?ie=utf-8&wd="+titleValue+"&cl=3\" target=\"_blank\"><img title=\"More details in Google\" alt=\"More details in Google\" style=\"width: 15px;height:15px;border: 0px;\" src=\"/gbdbas/static/img/datasearch/google-g-logo-vector.png\" ></a>"+"<font title='"+titleValue+"'>"+value+"</font>";
	}else
	{
		return "N/A";
	}
...
function saveQueryCon(){
	...
	{
  	    $.messager.alert("Prompt", "您填写的信息不够完整,请确认您的操作!","info");
	}
	else
	{
		...
		$.messager.alert("Prompt", "已添加,请在个人中心下我的收藏中找到它","info");
...
function validateNumericType(fieldName,fieldValue)
{
	...
	if (valueArray[0] != null && valueArray[0] != '' && valueArray[0] != 'undefined') {
		if (!regex.test(valueArray[0])) {
			...
			{
				alertMessage('Prompt', 'cif价格第一段文本框不是数字','info');
			}else if(fieldName == "fob_value")
			{
				alertMessage('Prompt', 'fob价格第一段文本框不是数字','info');
			...
			{
				alertMessage('Prompt', '数量第一段文本框不是数字','info');
	...
	if (valueArray[1] != null && valueArray[1] != '' && valueArray[1] != 'undefined') {
		if (!regex.test(valueArray[1])) {
			if(fieldName == "cif_value")
			{
				alertMessage('Prompt', 'cif价格第二段文本框不是数字','info');
			}else if(fieldName == "fob_value")
			{
				alertMessage('Prompt', 'fob价格第二段文本框不是数字','info');
			}else if(fieldName == "g_weight")
			{
			...
			{
				alertMessage('Prompt', '数量第二段文本框不是数字','info');
			}else if(fieldName == "packages")
			{
...
function validateDateType(fieldValue)
{
	...
	if (valueArray[0] == null || valueArray[0] == '') {
		alertMessage('Prompt', '第一段日期','info');
		return false;
	}
	//验证第二段日期
	if (valueArray[1] == null || valueArray[1] == '') {
		alertMessage('Prompt', '第二段日期','info');
		return false;
	}
	...
	if (startDate > endDate) {
		alertMessage('Prompt', '第二段日期不能小于第一段日期','info');
		return false;
	}
	
	// 验证时间段不超过三年
	if (timeBetween > 94608000000) {
		alertMessage('Prompt','查询最大的时间差不能超过三年!','info');
		return false;
	}
	return true;
}
...
function functionDaQuan(value, row, index)
{
	...
	{
		return '<a href = \'javascript:reportDetails(\"' + row.hideColumn + '\");\' title=\"报表明细"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/viewDetail.png\"></img></a>'
		+'&nbsp;'
		+'<a href = \'javascript:thbAnalysis(\"' + row.hideColumn + '\");\' title=\"同环比报告分析"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/THB.png\"></img></a>'
		+'&nbsp;'
		+'<a href = \'javascript:dataCompare(\"' + row.hideColumn + '\");\' title=\"数据对比"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/dataCompare.png\"></img></a>';
	}
}
...
function compareFunctionDaQuan(value, row, index)
{
	...
	{
		var daQuan =  '<a href = \'javascript:reportDetails(\"' + row.hideColumn + '\");\' title=\"报表明细"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/viewDetail.png\"></img></a>';
		if(row.hideColumn.split("罟")[1] != "qs")
		{
			daQuan+='&nbsp;'
			daQuan+='<a href = \'javascript:thbAnalysis(\"' + row.hideColumn + '\");\' title=\"同环比报告分析"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/THB.png\"></img></a>';
		}
		return daQuan;
	}
}
...
function functionMutiHscodeDaQuan(value, row, index)
{
	...
	}else
	{
		return '<a href = \'javascript:reportDetails(\"' + row.hideColumn + '\");\' title=\"报表明细"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/viewDetail.png\"></img></a>'
		+'&nbsp;'
		+ '<a href = \'javascript:mutiHscodeRemark(\"' + row.hideColumn + '\");\' title=\"多海关编码备注"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/remark.gif\"></img></a>'
		+'&nbsp;'
		+'<a href = \'javascript:dataCompare(\"' + row.hideColumn + '\");\' title=\"数据对比"\ style=\"color: #1369c0\" onmouseover="this.style.textDecoration=\'underline\'" onmouseout="this.style.textDecoration=\'none\'"><img src=\"/gbdbas/static/img/datasearch/dataCompare.png\"></img></a>';
	}
}
...
function saveCompareData()
{
	...
	{
		$.messager.alert("Prompt","请选择对比的数据!","info");
	}
}
	...
	for(var i=0;i<thbReportArray.length;i++)
	{
		...
		$('#'+thbReportArray[i]+''+'_thb_table').datagrid({
			...
			onLoadSuccess: function(data){
				...
				//打开同环比对话框
				openCompareDataDialog("同环比报告","thbDialog","thbDialog",900, 600,true);
				//混合图形div的id
